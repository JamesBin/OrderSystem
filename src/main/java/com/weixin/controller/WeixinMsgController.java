...
import com.weixin.sdk.jfinal.MsgController;
...
public class WeixinMsgController extends MsgController {
    ...
    private static final String helpStr = "发送 order 可获得当天菜单，发送 help 可获得帮助，发送\"福利\" 可获红包折扣 ";
    ...
    protected void processInTextMsg(InTextMsg inTextMsg) {
        ...
        if ("help".equalsIgnoreCase(msgContent) || "帮助".equals(msgContent)) {
            ...
            outMsg.setContent("如需帮助，请联系客服微信号：James377933019, 祝您订餐愉快！ ");
            render(outMsg);
        }
        // 图文消息测试
        else if ("order".equalsIgnoreCase(msgContent) || "菜单".equalsIgnoreCase(msgContent)) {
            OutNewsMsg outMsg = new OutNewsMsg(inTextMsg);
            outMsg.addNews("每日推荐菜色", "热辣辣黄焖鸡米饭", "http://p1.meituan.net/deal/5a6b4db1e51ea7bb4fb2af83cdd71250201069.jpg", "http://zwb163.6655.la/user");
//            outMsg.addNews("每日推荐菜色", "现在就订餐，享受更多实惠", "https://mmbiz.qlogo.cn/mmbiz/ibHRiaZ9MRcUosol56OtHjVibWTK9opiaxsY9tPDricojmV5xxuLJyibZJXMAdNOx1qbZFcic9SvsPF2fTUnSc9oQB1IQ/0?wx_fmt=jpeg","http://mp.weixin.qq.com/s?__biz=MzA4MDA2OTA0Mg==&mid=208413033&idx=1&sn=06e816e1b2905c46c9a81df0ac0b3bad#rd");
            render(outMsg);
        }
        // 音乐消息测试
...
//        }
        // 其它文本消息直接返回原值 + 帮助提示
            ...
            System.out.println("微信端接收内容为： " + inTextMsg.getContent());
    ...
    protected void processInFollowEvent(InFollowEvent inFollowEvent) {
        ...
        outMsg.addNews("小吉，没你的日子我可怎么过啊！", "热辣辣黄焖鸡米饭", "http://os.carp.mopaasapp.com/resources/img/anno/anno1.png", "http://mp.weixin.qq.com/s?__biz=MzA3NTcwNDcxOQ==&mid=423752035&idx=1&sn=e77ddb4a8d19ed73b14131402c10c107&scene=0#wechat_redirect");
        outMsg.addNews("重大通知：黄小吉微信官方服务新功能上线了【必看】", "现在就订餐，享受更多实惠", "http://os.carp.mopaasapp.com/resources/img/anno/anno2.jpg", "http://mp.weixin.qq.com/s?__biz=MzA3NTcwNDcxOQ==&mid=423752035&idx=2&sn=55c7ab2016ad7307534fcef0a6cdd00b&scene=0#wechat_redirect");
        render(outMsg);
    }
    ...
    public void processInLocationEvent(InLocationEvent inLocationEvent) {
        ...
        Location location = getModel(Location.class).findFirst("select * from location where fromUserName = ?", inLocationEvent.getFromUserName());
        if (location != null) {
            Db.update("delete from location where fromUserName = ?", inLocationEvent.getFromUserName());
        }

        getModel(Location.class).set("toUserName", inLocationEvent.getToUserName())
                .set("fromUserName", inLocationEvent.getFromUserName())
                .set("createTime", inLocationEvent.getCreateTime())
                .set("msgType", inLocationEvent.getMsgType())
                .set("event", inLocationEvent.getEvent())
                .set("latitude", inLocationEvent.getLatitude())
                .set("longitude", inLocationEvent.getLongitude())
                .set("precision", inLocationEvent.getPrecision())
                .save();
